      ...
      <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-6'>
        {
            doctors.slice(0,10).map((ele,index)=>(
                <div onClick={()=> navigate(`/appointment/${ele._id}`)} key={index} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
